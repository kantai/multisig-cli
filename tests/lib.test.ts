...
test('Transaction building (success)', async () => {
  ...
  it('Should have correct pubkeys', () => {
    spendingCondition.fields.forEach((f, i) => {
      ...
      expect(pubkey).toEqual(signers[i])
    });
  });
...
test('Transaction building from array (success)', async () => {
  ...
  const inputs: lib.MultisigTxInput[] = [
    { recipient, fee: '300', amount: '10000', signers, reqSignatures: 3, nonce: '4', network: 'mainnet' }
  ];

  const tx = await lib.makeTxsFromInputs(inputs);
  const spendingCondition = tx[0].auth.spendingCondition as StxTx.MultiSigSpendingCondition;

  it('Should have correct numbers of auth fields', () => {
    const info = lib.getAuthFieldInfo(tx[0]);
    expect(info).toEqual({
      authFields: 3,
      pubkeys: 3,
      signatures: 0,
